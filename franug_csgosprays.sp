...
public Action:OnBombPlanted(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if (IsFakeClient(user)) {
        int probability = GetRandomInt(1, 100);
        if (probability >= 20) {
            MakeSpray(user, 1);
        }
    }
...
public Action BotSprayTimer(Handle timer) {
